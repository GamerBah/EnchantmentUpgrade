...
public class Updater {
	...
	public Updater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback, boolean announce) {
		...
		try {
			...
			this.plugin.getLogger()
					.log(Level.SEVERE, "The project ID provided for updating, " + this.id + " is invalid.", e);
	...
	private void fileIOOrError(File file, boolean result, boolean create) {
		if (!result) {
			this.plugin.getLogger()
					.severe("The updater could not " + (create ? "create" : "delete") + " file at: " + file
							.getAbsolutePath());
		}
	}
	...
	private void unzip(String file) {
		...
		try {
			...
			while (e.hasMoreElements()) {
				...
				this.fileIOOrError(
						destinationFilePath.getParentFile(),
			...
			this.plugin.getLogger()
					.log(Level.SEVERE, "The auto-updater tried to unzip a new update file, but was unsuccessful.", e);
			this.result = Updater.UpdateResult.FAIL_DOWNLOAD;
		} finally {
	...
	private boolean versionCheck() {
		...
		if (this.type != UpdateType.NO_VERSION_CHECK) {
			...
			if (title.split(DELIMETER).length >= 2) {
				...
				final String authorInfo = this.plugin.getDescription().getAuthors().isEmpty() ? "" : " (" + this.plugin
